...
class SendMarketSMS extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'unified:send-market-sms';
     ...
     * @var string
     */
    protected $description = 'Send already composed market info messages';
    ...
    {
        if ($this->debug) Log::info(['Command' => 'Sending market information']);

        try {
            MarketOutbox::where('status', 'PENDING')
                ...
                ->chunk(500, function ($messages) {
                    ...
                    MarketOutbox::whereIn('id', $messages->pluck('id')->toArray())->update(['status' => 'SELECTED']);
                    ...
                    foreach ($messages as $message) {
                        ...
                        MarketSubscription::where('id', $message->subscription_id)->update([
            ...
            Log::error(['SendMarketSMS' => $r->getMessage()]);            
        } 
    }
}
