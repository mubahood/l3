...
class MtnUganda
{
    ...
    {
        $dedit                      = config('payments.services.mtn_debit_ug.');
        $sptransfer                 = config('payments.services.mtn_sptransfer_ug.');
    ...
    /**
     * Sends an XML request via cURL and returns the resulting handle.
     *
    ...
    public function postXml($xml, $username, $password, $api_url) {
        // Combine the username and password into a single string and encode it.
        $auth = $username . ':' . $password;
        $credentials = base64_encode($auth);

        // Set the headers to include the content type and authorization.
        $headers = array();
    ...
    /**
     * Deposit funds to a specified MSISDN
     * @param string $msisdn - MSISDN of the recipient
     * @param float $amount - Amount to be transferred
     * @param string $narrative - Narrative message for the transaction
     * @param string $reference - External transaction ID
     * @return Request - An instance of Request model with transaction details
     */
    public function depositFunds($msisdn, $amount, $narrative, $reference = null)
    {
        // Create the XML request for debit transaction
        $xml = '<?xml version="1.0" encoding="UTF-8"?>';
        $xml .= '<ns0:debitrequest xmlns:ns0="http://www.ericsson.com/em/emm/financial/v1_0">';
        $xml .= '<fromfri>FRI:' . $msisdn . '/MSISDN</fromfri>';
        $xml .= '<tofri>FRI:' . $this->_sp_debit_username . '/USER</tofri>';
        $xml .= '<amount>';
        $xml .= '<amount>' . $amount . '</amount>';
        $xml .= '<currency>UGX</currency>';
        $xml .= '</amount>';
        $xml .= '<externaltransactionid>' . $reference . '</externaltransactionid>';
        $xml .= '<frommessage>' . $narrative . '</frommessage>';
        $xml .= '<referenceid>' . $reference . '</referenceid>';
        $xml .= '</ns0:debitrequest>';

        // Log debit request to a file
        Log::useFiles(base_path() . '/storage/logs/ecw_debit.log');
        Log::info(['debitRequest' => $xml]);

        // Send the XML request using curl and get the response
        $curl_request = $this->postXml($xml, $this->_sp_debit_username, $this->_sp_debit_password, '/v1/debit');
        $response = curl_exec($curl_request);

        // Initialize variables to handle response data
        $result = null;
        $status = 'FAILED';
        $transactionid = null;
        $errorResponseMsg = '';
        ...
        if (curl_error($curl_request) || $response === false) {
            ...
            // Handle response data if there was no error
    ...
    * <ns0:errorResponse xmlns:ns0="http://www.ericsson.com/lwac" errorcode="INTERNAL_ERROR"/>
    * <?xml version="1.0" encoding="UTF-8"?>
    * <ns0:errorResponse xmlns:ns0="http://www.ericsson.com/lwac" errorcode="REFERENCE_ID_ALREADY_IN_USE"/>
    * <?xml version="1.0" encoding="UTF-8"?>
    * <ns0:errorResponse xmlns:ns0="http://www.ericsson.com/lwac" errorcode="TARGET_AUTHORIZATION_ERROR"/>
    *
    */
