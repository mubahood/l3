...
class MP3File
{
    ...
    public static function parseFrameHeader($fourbytes)
    {
        ...
        $info['Samples'] = $samples[$simple_version][$layer] ?? 0;
        return $info;
    }
    ...
    {
        if ($layer==1)
            return $sample_rate == 0 ? 0 : intval(((12 * $bitrate*1000 /$sample_rate) + $padding_bit) * 4);
        else //layer 2, 3
            return $sample_rate == 0 ? 0 : intval(((144 * $bitrate*1000)/$sample_rate) + $padding_bit);
    }
}