            ...
            <div class="card-body">
                <ul class="nav nav-tabs" role="tablist">  
                    ...
                    @can('view_el_forum_topics')
                        <li class="nav-item"><a class="nav-link active" href="{{ url()->current() }}">Discussion</a></li>
                    @endcan
                            ...
                            <div class="col-md-12">
                                <section class="event card border">
                                    <div class="d-flex">
                                        ...
                                        <div>
                                            <h4 class="event-heading"><a href="#">{{ $data->user->name }}</a><span><small class="text-muted"><a href="#">&nbsp;{{ $data->user->telephone }}</a></small></span></h4>
                                        ...
                                        <div>
                                            <div class="clearfix border-top post-comments">
                                                ...
                                                @can('add_el_forum_topic_subscriptions')
                                                    @if ($data->hasSuscribed())
                                                        <a href="#" class="btn btn-default btn-sm float-right mr-2 mt-2">Subscribed</a>
                                                    @else
                                                        <a href="{{ url('e-learning/courses/forums/topics/'.$data->id.'/subscribe') }}" class="btn btn-warning btn-sm float-right mr-2 mt-2">Subscribe</a>
                                                    @endif
                                                @endcan
                                                @can('add_el_forum_topic_likes')
                                                    @if ($data->hasLiked())
                                                        <a href="#" class="btn btn-default btn-sm float-right mr-2 mt-2">You Liked</a>
                                                    @else
                                                        <a href="{{ url('e-learning/courses/forums/topics/'.$data->id.'/like') }}" class="btn btn-success btn-sm float-right mr-2 mt-2">Like</a>
                                                    @endif
                                                @endcan
                                            </div>
                                        </div>
                                    ...
                                    <section class="event card border">
                                        ...
                                        <h4 class="mt-3">Responses
                                            @can('add_el_forum_topic_responses')
                                                <a href="{{ url('e-learning/courses/forums/topics/'.$data->id.'/respond') }}" class="btn btn-primary btn-sm float-right">Respond</a>
                                            @endcan
                                        </h4>
                                        <div>
                                                    ...
                                                    @foreach ($data->responses as $response)
                                                        <li class="d-flex">
                                                            ...
                                                            <div class="comment-body">
                                                                <h4 class="author fw-semi-bold">{{ $data->user->name }} <br/><small class="text-muted">{{ date('d F Y, h:i A', strtotime($response->created_at)) }}</small></h4>
                                                                ...
                                                                @if(!is_null($response->audio_url))
                                                                    <div>
                                                                        <p>&nbsp;</p>
                                                                        <audio controls class="col-12 mb-1">
                                                                            <source src="{{ asset('uploads/courses/'.$response->audio_url) }}">
                                                                        </audio>
                                                                    </div>
                                                                @endif
                                                            </div>
                                                                ...
                                                                <div class="pull-right" style="margin-top:-45px">
                                                                    ...
                                                                    @can('add_el_forum_topic_likes')
                                                                        @if ($response->hasLiked())
                                                                            <a href="#" class="btn btn-default btn-sm float-right mr-2 mt-2">You Liked</a>
                                                                        @else
                                                                            <a href="{{ url('e-learning/courses/forums/topics/responses/'.$response->id.'/like') }}" class="btn btn-success btn-sm float-right mr-2 mt-2">Like</a>
                                                                        @endif
                                                                    @endcan
                                                                </div>
                                                            </div>
