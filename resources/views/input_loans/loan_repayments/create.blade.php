@inject('set', 'App\Http\Controllers\InputLoan\LoanRepaymentController')
@extends('layouts.app')
                                    ...
                                    <div class="col-md-12">
                                        <div class="row">
                                            <div class="col-md-6 p-1">
                                                {!! Form::label('farmer_group_id', 'Farmer Group*', ['class' => 'col-sm-6 col-form-label']) !!}                
                                                <div class="col-sm-10">
                                                    <select class="form-control select2" name="farmer_group_id">
                                                        @php($groups=[])
                                                        @foreach($groups as $group)
                                                            <option value="{{ $group->id }}">{{ $group->group_name . ' (' . $group->group_leader . ', ' . $group->establishment_year . ')' }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
                                            <div class="col-md-6 p-1">
                                                {!! Form::label('microfinance_id', 'Microfinance*', ['class' => 'col-sm-6 col-form-label']) !!}                
                                                <div class="col-sm-10">
                                                    <select class="form-control select2" name="microfinance_id">
                                                        @php($finances=[])
                                                        @foreach($finances as $fin)
                                                            <option value="{{ $fin->id }}" title="{{ $fin->description }}">{{ App\Models\User::where('id',$fin->user_id)->first()->name . ' (Code: ' . $fin->code . ', ' . $fin->telephone_contact .')' }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                            </div>
